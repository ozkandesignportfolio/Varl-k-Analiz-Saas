...
async def run_test():
    ...
    try:
        ...
        # -> Click the 'Giriş Yap' (Login) link to open the login page (interactive element index 120).
        ...
        elem = frame.locator('xpath=/html/body/main/div[2]/section[2]/form/label[1]/input').nth(0)
        assert await elem.input_value() == os.getenv('TEST_LOGIN_EMAIL', '')
        elem = frame.locator('xpath=/html/body/main/div[2]/section[2]/form/label[2]/input').nth(0)
        assert await elem.input_value() == os.getenv('TEST_LOGIN_PASSWORD', '')
        elem = frame.locator('xpath=/html/body/main/div[2]/section[2]/form/button').nth(0)
        assert await elem.is_visible()
        raise AssertionError('Feature missing: Services/New Service Entry and cost field not found on the current page; cannot perform negative-cost validation.')
