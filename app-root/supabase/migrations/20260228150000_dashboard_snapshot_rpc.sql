...
create index if not exists idx_assets_user_updated_at_desc
  on public.assets(user_id, updated_at desc);

create index if not exists idx_assets_user_created_at_asc
  on public.assets(user_id, created_at asc);

create index if not exists idx_service_logs_user_created_at_desc
  on public.service_logs(user_id, created_at desc);

create index if not exists idx_maintenance_rules_user_created_at_desc
  on public.maintenance_rules(user_id, created_at desc);

create index if not exists idx_maintenance_rules_user_next_due_date_asc
  on public.maintenance_rules(user_id, next_due_date asc)
  where is_active;

create index if not exists idx_documents_user_asset_id
  on public.documents(user_id, asset_id);

create index if not exists idx_documents_uploaded
  on public.documents(user_id, uploaded_at desc);

create index if not exists idx_assets_user_warranty_end_date_asc
  on public.assets(user_id, warranty_end_date asc)
  where warranty_end_date is not null;

create index if not exists idx_billing_invoices_user_due_pending_overdue
  on public.billing_invoices(user_id, due_date asc, created_at asc)
  where status in ('pending', 'overdue') and due_date is not null;

create index if not exists idx_billing_invoices_user_paid_at_desc
  on public.billing_invoices(user_id, paid_at desc)
  where status = 'paid' and paid_at is not null;

create or replace function public.dashboard_build_trend(p_current numeric, p_previous numeric)
returns jsonb
language sql
immutable
as $$
with base as (
  select
    coalesce(p_current, 0)::numeric as current_value,
    coalesce(p_previous, 0)::numeric as previous_value
),
delta as (
  select
    current_value,
    ...
    case
      when current_value <= 0 and previous_value <= 0 then 'flat'
      when previous_value <= 0 and current_value > 0 then 'up'
      when current_value - previous_value > 0 then 'up'
      when current_value - previous_value < 0 then 'down'
      else 'flat'
    end as direction,
    case
      when current_value <= 0 and previous_value <= 0 then 0
      when previous_value <= 0 and current_value > 0 then 100
      else least(
        999,
        round(
          abs(current_value - previous_value)
          / abs(case when previous_value = 0 then 1 else previous_value end)
          * 100
        )
      )::int
    end as percentage
...
select jsonb_build_object(
  ...
  'sparkline',
  case direction
    when 'up' then '[22,24,28,30,34,37]'::jsonb
    when 'down' then '[37,34,31,28,25,22]'::jsonb
    else '[28,27,28,27,28,27]'::jsonb
  end
)
from delta;
$$;
    ...
    timeline_events as (
      ...
      from (
        select
          ...
          'Servis kaydi eklendi'::text as title,
          (coalesce(a.name, 'Bilinmeyen Varlik') || ' - ' || sl.service_type)::text as description,
      ...
      from (
        select
          ...
          'Belge yuklendi'::text as title,
          (coalesce(a.name, 'Bilinmeyen Varlik') || ' - ' || d.file_name)::text as description,
      ...
      from (
        select
          ...
          'Bakim kurali olusturuldu'::text as title,
          (coalesce(a.name, 'Bilinmeyen Varlik') || ' - ' || r.title)::text as description,
      ...
      select
        case
          ...
          when si.overdue_maintenance_count > 0 then
            jsonb_build_object(
              'tone', 'critical',
              'headline', si.overdue_maintenance_count::text || ' gecikmis bakim',
          ...
          when si.overdue_payment_count > 0 then
            jsonb_build_object(
              'tone', 'critical',
              'headline', si.overdue_payment_count::text || ' gecikmis odeme',
          ...
          when (si.upcoming_maintenance_count + si.upcoming_warranty_count + si.upcoming_payment_count + si.missing_documents_count) > 0 then
            jsonb_build_object(
              'tone', 'warning',
              'headline', (si.upcoming_maintenance_count + si.upcoming_warranty_count + si.upcoming_payment_count + si.missing_documents_count)::text || ' risk var',
