  ...
  return (
    ...
    >
      <div className="flex items-center justify-between gap-2">
        <div>
          <p className="text-xs uppercase tracking-[0.16em] text-[#90A6C4]">Kullanım</p>
      ...
      <div className="mt-4 space-y-3">
        {items.map((item) => {
          ...
          return (
            <article key={item.id} className="rounded-xl border border-[#314866] bg-[#0F1E37]/75 p-3" data-testid={`plan-usage-item-${item.id}`}>
      ...
      <p className="mt-4 text-sm text-[#9FB2CE]">
        {isPremium
          ? "Premium planınız aktif. Tüm limitler kaldırıldı ve gelişmiş özellikler kullanımda."
          : "Premium ile tüm limitleri kaldırın ve otomasyon ile raporlamayı tam kapasiteye taşıyın."}
      </p>

      {isPremium ? (
        <div
          data-testid="dashboard-plan-premium-status"
          className="mt-4 inline-flex w-full items-center justify-center gap-2 rounded-xl border border-emerald-300/35 bg-emerald-300/10 px-4 py-2.5 text-sm font-semibold text-emerald-100"
        >
        ...
        <Link
          href="/pricing"
          data-testid="dashboard-plan-upgrade-button"
