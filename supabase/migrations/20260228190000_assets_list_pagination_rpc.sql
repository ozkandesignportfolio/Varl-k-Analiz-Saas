...
create index if not exists idx_assets_user_updated_at_id_desc
  on public.assets(user_id, updated_at desc, id desc);

create index if not exists idx_service_logs_user_asset_service_date_desc
  on public.service_logs(user_id, asset_id, service_date desc, id desc);

create index if not exists idx_documents_user_asset_uploaded_at_desc
  on public.documents(user_id, asset_id, uploaded_at desc, id desc);

create index if not exists idx_maintenance_rules_user_asset_due_active
  on public.maintenance_rules(user_id, asset_id, next_due_date asc)
...
as $$
  ...
  where a.user_id = p_user_id
    and nullif(btrim(a.category), '') is not null
...
create or replace function public.list_asset_activity_preview(
  p_user_id uuid,
  p_asset_ids uuid[],
  p_per_asset_limit integer default 3
)
returns table(
  asset_id uuid,
  id uuid,
...
as $$
  with ranked as (
    select
      ...
      sl.id,
      sl.service_type,
      sl.service_date::date as service_date,
      coalesce(sl.cost, 0)::numeric as cost,
      row_number() over (partition by sl.asset_id order by sl.service_date desc, sl.id desc) as rn
    from public.service_logs sl
    where sl.user_id = p_user_id
      and sl.asset_id = any(p_asset_ids)
  )
  select
    r.asset_id,
    r.id,
    ...
    r.cost
  from ranked r
  where r.rn <= greatest(1, least(coalesce(p_per_asset_limit, 3), 10))
  order by r.asset_id, r.service_date desc, r.id desc;
$$;
