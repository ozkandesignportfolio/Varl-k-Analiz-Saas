...
async def run_test():
    ...
    try:
        ...
        # -> Navigate to /login using explicit navigation (test step requires direct navigate to /login).
        await page.goto("http://localhost:3000/login", wait_until="commit", timeout=10000)
        
        # -> Type $TEST_LOGIN_EMAIL into the E-posta field, type $TEST_LOGIN_PASSWORD into the Şifre field, then click 'Giriş Yap' (Sign in).
        frame = context.pages[-1]
        # Input text
        ...
        frame = context.pages[-1]
        await expect(frame.locator('text=Next due').first).to_be_visible(timeout=3000)
