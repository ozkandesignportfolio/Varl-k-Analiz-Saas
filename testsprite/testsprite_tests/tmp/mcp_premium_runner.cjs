...
const OUTPUT_DIR = path.join(process.cwd(), 'testsprite', 'testsprite_tests', 'tmp');
...
async function main() {
  ...
  try {
    ...
    await page.getByRole('button', { name: /Giri[sï¿½] Yap|Login/i }).first().click();
    ...
    report.login = {
      ...
      dashboardHeadingVisible: await page.getByText(/Gï¿½sterge|Dashboard/i).first().isVisible().catch(() => false),
    };
    ...
    await runFlow('create_asset', async () => {
      ...
      await page.waitForTimeout(1200);
      await page.click('#asset-create-form button:has-text("Varlï¿½ï¿½ï¿½ Kaydet")');
      ...
      const ok = assetVisible || /baï¿½arï¿½yla|eklendi|kayded/i.test(assetFeedback || '');
      if (ok) {
        return {
          ok,
    ...
    await runFlow('create_maintenance_rule', async () => {
      ...
      await page.waitForTimeout(1200);
      const modalVisible = await page.getByText(/2 adï¿½mda kural oluï¿½turun|1\\. Varlï¿½k ve isim/i).first().isVisible().catch(() => false);
      if (!modalVisible) {
      ...
      await page.getByRole('button', { name: /Kuralï¿½ Oluï¿½tur/i }).click().catch(() => {});
      await page.waitForTimeout(1800);
      const maintenanceFeedback = await page.locator('p.rounded-xl').first().textContent().catch(() => '');
      return {
        ok: modalVisible || /oluï¿½turuldu|gï¿½ncellendi|bakï¿½m kuralï¿½/i.test((maintenanceFeedback || '').toLowerCase()),
    ...
    await runFlow('create_service_log', async () => {
      ...
      const submitBtn = page.getByRole('button', { name: /Servis Kaydï¿½nï¿½ Ekle/i });
      ...
      return {
        ok: /servis kaydï¿½ eklendi|eklendi/i.test((serviceFeedback || '').toLowerCase()),
    ...
    if (canSwitchPlan) {
      ...
      if (toTrial.ok) {
        ...
        const gateVisible = await page
          .getByText(/Premium ï¿½zellik|Premium Ã–zellik|Premium'da aktif|yalnï¿½zca Premium|Trial planda/i)
          .first()
          .isVisible()
          .catch(() => false);
