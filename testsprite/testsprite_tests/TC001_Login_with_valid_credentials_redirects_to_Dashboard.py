...
async def run_test():
    ...
    try:
        ...
        # -> Navigate to the login page using the explicit path /login (http://localhost:3000/login).
        await page.goto("http://localhost:3000/login", wait_until="commit", timeout=10000)
        
        # -> Fill the email field (index 1861) with $TEST_LOGIN_EMAIL, fill the password field (index 1862) with $TEST_LOGIN_PASSWORD, then click the Login button (index 1863).
