...
async def run_test():
    ...
    try:
        ...
        # -> Type os.getenv('TEST_LOGIN_EMAIL', '') into the email field, type os.getenv('TEST_LOGIN_PASSWORD', '') into the password field, then click the 'Giriş Yap' (Sign in) button.
        frame = context.pages[-1]
        # Input text
        ...
        await expect(frame.locator('text=Oil change performed').first).to_be_visible(timeout=3000)
