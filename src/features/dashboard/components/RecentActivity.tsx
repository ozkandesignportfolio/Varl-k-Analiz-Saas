    ...
    <section className="rounded-2xl border border-[#2B3F5D] bg-[linear-gradient(145deg,rgba(10,22,44,0.92),rgba(11,18,35,0.84))] p-5 shadow-[0_16px_34px_rgba(2,8,20,0.34)]">
      ...
      {items.length === 0 ? (
        <div className="mt-4 rounded-xl border border-[#314866] bg-[#0E1E37]/75 px-4 py-6 text-center">
          ...
          <p className="mt-2 text-sm text-[#9FB2CE]">Servis, belge veya ödeme işlemleri burada zaman sıralı görünecek.</p>
        </div>
      ) : (
        <ol className="mt-4 space-y-2.5">
          {items.map((item) => {
            ...
            return (
              <li key={item.id} className="rounded-xl border border-[#314866] bg-[#0E1E37]/75 p-3">
                <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
                  ...
                  <div className="flex items-center gap-3">
                    <span className="text-xs text-[#9FB2CE]">
                      {DATE_TIME_FORMATTER.format(new Date(item.date))}
                    </span>
                    <Link
                      href={item.href}
